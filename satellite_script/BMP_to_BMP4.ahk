	; 	Imagemagick command line convert bmp to bmp4 format
#NoEnv ; #Persistent  
...
Menu, tray, standard
menu, tray, tip , bmp4 conversion for:`n%FileName%
...
if errorlevel {
	...
	{		
		...
		{
			msgbox, bmp4 Already Exists
			exit
	  } else {
			run %COMSPEC% /C magick convert "%File%" BMP4:"%NewFile%",,, hide
			sleep % S
			...
			trayTip, bmp 2 bmp, bmp4 replaced & in clipboard.
		}
		...
		msgbox, 0, %FileName% to bmp4, %FileName%`nConverted to bmp4 & in clipboard.`nThis message will self destruct in 3 seconds,3
		exit
	}
	return
}
